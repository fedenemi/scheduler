...
static int
sys_increase_priority(envid_t envid, uint32_t new_tickets)
{
	struct Env *env;
	int r;
	if ((r = envid2env(envid, &env, true)) < 0) {
	...
	uint32_t total_ticks = new_tickets + env->tickets;

	if (total_ticks <= 100000) {
		env->tickets = total_ticks;
	} else {
		return -1;
	}
	return env->tickets;
...
static int
sys_decrease_priority(envid_t envid, uint32_t less_tickets)
{
	struct Env *env;
	int r;
	if ((r = envid2env(envid, &env, true)) < 0) {
	...
	if (env->tickets > less_tickets) {
		env->tickets -= less_tickets;
	} else {
		return -1;
	}
	return env->tickets;
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_decrease_priority:
		return sys_decrease_priority(a1, a2);
	case SYS_increase_priority:
		return sys_increase_priority(a1, a2);
	default:
		return -E_INVAL;
	}
}
